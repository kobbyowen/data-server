...
class TestMain(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.argument_parser_instance.get_parsed_arguments.return_value = {
            ...
            'use_timestamps': True,
            'created_at_key_name': 'created_at',
            'updated_at_key_name': 'updated_at',
            'page_size': 10,
            'url_path_prefix': '/',
            'host': 'localhost',
            'port': 2020,
    ...
    def test_main(self) -> None:
        ...
        server.run()
        self.assertTrue(self.argument_parser_instance.get_parsed_arguments.called)
        self.assertTrue(self.server_instance.run.called)

    def test_run_server_throwing_exception(self) -> None:
        self.server_instance.run.side_effect = DataServerError('Failed to start server')
