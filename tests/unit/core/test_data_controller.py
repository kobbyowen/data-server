...
from tests.unit.fake_data import (
    data_sample,
    data_sample_with_empty_posts,
    data_sample_with_id_name_as_uuid,
    data_sample_with_int_ids,
    data_sample_with_nested_items,
    data_sample_with_string_ids,
)
...
class TestGetItems(unittest.TestCase):
    def test_get_items(self) -> None:
        ...
        self.assertDictEqual(items[0], {'id': 1, 'author': 'Kobby Owen', 'title': 'Advanced Python'})
...
class TestAddItem(unittest.TestCase):
    ...
    def test_add_item_with_existing_id(self) -> None:
        ...
        with self.assertRaises(DuplicateIDFoundError):
...
class TestAutoGenerateId(unittest.TestCase):
    ...
    def test_auto_generate_int_ids(self) -> None:
        ...
        generated_id = controller._autogenerate_id(t.cast(dt.JSONItems, self.data_sample_with_ints_ids['books']))
        expected_ids = [2, 3]
    ...
    def test_autogenerate_id_with_single_data(self) -> None:
        controller = DataController(self.data_sample_with_ints_ids, autogenerate_id=True)
        generated_id = controller._autogenerate_id(t.cast(dt.JSONItems, [self.data_sample_with_ints_ids['books'][0]]))
        expected_ids = [2, 3]
        self.assertIn(generated_id, expected_ids)
        self.assertNotEqual(generated_id, 1)
    ...
    def test_auto_generate_int_ids_randomly(self, mocked_randint: MagicMock) -> None:
        ...
        generated_id = controller._autogenerate_id(
            t.cast(dt.JSONItems, self.data_sample_with_ints_ids['books']), use_random=True
        )
...
class TestFixData(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.data_sample_with_ints_ids = t.cast(dt.JSONItem, deepcopy(sample_data_1))
        self.data_sample_with_string_ids = t.cast(dt.JSONItem, deepcopy(sample_data_2))
        return super().setUp()

    def test_adding_automatic_integer_ids(self) -> None:
        DataController(self.data_sample_with_ints_ids, fix=True, use_timestamps=True)
    ...
    def test_adding_automatic_string_ids(self) -> None:
        DataController(self.data_sample_with_string_ids, fix=True, use_timestamps=True)
    ...
    def test_removing_timestamps(self) -> None:
        DataController(self.data_sample_with_string_ids, fix=True, use_timestamps=False)
        for book in self.data_sample_with_string_ids['books']:
