...
def default_handler(method: str, url: str, args: t.Any, data: t.Any) -> dt.RouterResponse:
    return {'url': url, 'method': method, 'args': dict(args), 'data': dict(data)}
...
class TestServerInitialization(TestCase):
    ...
    def test_initialization_with_disabled_logs_and_stdin(self, mocked_logger: MagicMock) -> None:
        ...
        server = Server(default_handler, disable_logs=True, disable_stdin=True)
...
class TestRequesthandling(TestCase):
    ...
    def test_request_with_additional_headers(self) -> None:
        ...
        self.response_adapter_mock.assert_called_with(
            target_response,
            status=200,
            headers={'Access-Control-Allow-Origin': '*', 'X-Range': '20', 'X-Limit': '30'},
            mimetype='application/json',
        )
