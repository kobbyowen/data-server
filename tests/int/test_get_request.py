...
class TestGetRequest(GetRequestTestCase):
    def test_get_all_orders_request(self) -> None:
        ...
        self.assertListEqual(t.cast(t.List[t.Any], response['json']), self.sorted_orders[: self.server.page_size])


class TestPaginatedGetRequest(GetRequestTestCase):
    def test_get_request_with_valid_page_and_size(self) -> None:
        response = self.client.get(f'/api/v3/orders?{self.server.page_param_name}=0&{self.server.size_param_name}=10')
        self.assertEqual(response['status'], 200)
        self.assertListEqual(t.cast(t.List[t.Any], response['json']), self.sorted_orders[:10])

    def test_get_request_with_invalid_page(self) -> None:
        response = self.client.get(f'/api/v3/orders?{self.server.page_param_name}=-1&{self.server.size_param_name}=10')
        self.assertEqual(response['status'], 400)
        self.assertIn('error', response['json'])
        response = self.client.get(f'/api/v3/orders?{self.server.page_param_name}=A&{self.server.size_param_name}=10')
        self.assertEqual(response['status'], 400)
        self.assertIn('error', response['json'])
    ...
    def test_get_request_with_invalid_size(self) -> None:
        ...
        self.assertEqual(response['status'], 400)
        self.assertIn('error', response['json'])
        response = self.client.get(f'/api/v3/orders?{self.server.page_param_name}=0&{self.server.size_param_name}=A')
        self.assertEqual(response['status'], 400)
        self.assertIn('error', response['json'])

    def test_get_request_with_missing_size(self) -> None:
        response = self.client.get(f'/api/v3/orders?{self.server.page_param_name}=0')
        self.assertEqual(response['status'], 200)
        self.assertListEqual(t.cast(t.List[t.Any], response['json']), self.sorted_orders[: self.server.page_size])
    ...
    def test_get_request_with_large_page_number(self) -> None:
        response = self.client.get(f'/api/v3/orders?{self.server.page_param_name}=100000')
        self.assertEqual(response['status'], 200)
        self.assertListEqual(t.cast(t.List[t.Any], response['json']), [])

    def test_get_request_with_large_size_number(self) -> None:
        response = self.client.get(f'/api/v3/orders?{self.server.size_param_name}=100000')
        self.assertEqual(response['status'], 200)
        self.assertListEqual(t.cast(t.List[t.Any], response['json']), self.sorted_orders)
    ...
    def test_pagination_navigation(self) -> None:
        for i in range(0, 10):
            ...
            self.assertEqual(response['status'], 200)
            self.assertEqual(response['json'], self.sorted_orders[i * 2 : (i * 2) + 2])
...
class TestSortedGetRequest(GetRequestTestCase):
    ...
    def test_get_request_with_a_different_sort_key(self) -> None:
        response = self.client.get(f'/api/v3/orders?{self.server.sort_param_name}=orderNumber')
        self.assertEqual(response['status'], 200)
        sorted_orders = self.orders[:]
        sorted_orders.sort(key=lambda order: order['orderNumber'])
        self.assertListEqual(t.cast(t.List[t.Any], response['json']), sorted_orders[: self.server.page_size])

    def test_get_request_with_an_invalid_sort_key(self) -> None:
        response = self.client.get(f'/api/v3/orders?{self.server.sort_param_name}=noAttribute')
        sorted_orders = self.orders[:]
        sorted_orders.sort(key=lambda order: order['createdAt'])
        self.assertEqual(response['status'], 200)
        # maintain same order
        self.assertListEqual(t.cast(t.List[t.Any], response['json']), self.orders[: self.server.page_size])
    ...
    def test_get_request_with_sorting_in_descending_order(self) -> None:
        ...
        self.assertEqual(response['status'], 200)
        sorted_orders = self.orders[:]
        sorted_orders.sort(key=lambda order: order['orderNumber'], reverse=True)
        self.assertListEqual(t.cast(t.List[t.Any], response['json']), sorted_orders[: self.server.page_size])
