...
class TestServer:
    def __init__(self, port: int, server_file: str = 'tests/int/fixtures/server-data.json', **server_options) -> None:
    ...
    def run(self) -> None:
        ...
        --additional-headers='{self.additional_headers}' --id-name={self.id_name}  \
        ...
        {'--auto-generate-ids=true' if self.auto_generate_id else '--auto-generate-ids=false'} \
...
class TestClient:
    ...
    def _make_request(
        ...
        if (
            method in {dt.HTTPMethod.POST, dt.HTTPMethod.PUT, dt.HTTPMethod.PATCH, dt.HTTPMethod.DELETE}
...
def generate_order(
    ...
    **kwargs: t.Any,
    ...
    order_data = {
        id_name: randint(0, random_number_upper_limit) if id_type is int else str(uuid4()),
