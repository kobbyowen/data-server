...
from data_server.errors import DataControllerError
...
class DataController:
    def __init__(
        ...
        Args:
            ...
            of the developer to use deepcopy to copy the data before passing it , if modifications are not allowed
            ...
            Defaults to "page".
            size_param_name (Text, optional): The key that can be used to control the size for pagination.
            Defaults to "size".
    ...
    def replace_item(self, path: dt.ItemPath, id: dt.IdType,
        ...
        """Replace and item's old values with new values in new_data parameter, unlike patch,
    ...
    def _get_item_parent_and_index(
        ...
        try:
            ...
            raise ItemNotFoundError(
                f"item with id {id} could not be resolved from path {path!r}")
    ...
    def _get_items(self, data: dt.JSONItems,
        ...
        self.page = int(filters.pop(self.page_param_name, 0))
        if not isinstance(self.page, int) or self.page < 0:
            raise DataControllerError(
                f"{self.page_param_name!r} should be an non negative integer, got {self.page}",
                400)
        self.size = int(filters.pop(
            self.size_param_name, self.default_page_size))
        if not isinstance(self.size, int) or self.size < 0:
            raise DataControllerError(
                f"{self.size_param_name!r} should be an non negative integer, got {self.size}",
                400)
